...
- FR-6 No grounded match → L2 handoff screen with contact cards; conversation status becomes `handoff`.
...
- FR-13 Attachments (documents/images, session memory only, capped) are supported by the backend; the UI is currently commented out.
...
- NFR-7 Security: no secrets in the repo; CORS restricted to configured origins; `Content-Disposition` exposed for downloads.

## 5a. Out of scope (POC)
Real LLM/vector search, persistent database, real SSO/JWT, multi-tenant admin, live L2 chat.
...
3. A nonsense support question triggers the handoff screen; "hi" does not.
